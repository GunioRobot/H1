...
public class ZkClock implements Clock {
	...
	public static final String RETRY_DELAY_PROPERTY = "com.talis.platform.sequencing.zookeeper.retrydelay";
	public static final String RETRY_COUNT_PROPERTY = "com.talis.platform.sequencing.zookeeper.retrycount";
	...
	public long getNextSequence(String key) throws SequencingException {
		...
		for (int i = 0; i < retryCount; i++) {
			...
			try {
				...
				LOG.error(String.format("Caught an unexpected error when "
						+ "incrementing sequence fopr key %s", key), e);
				if (exception == null) {
					exception = e;
				}
			}
		...
		throw new SequencingException(String.format(
				"Failed to obtain next sequence for key %s", key), exception);
	}
	...
	private void createKey(String key) throws KeeperException {
		...
		try {
			myZooKeeper.create(key, DEFAULT_DATA, DEFAULT_ACL,
